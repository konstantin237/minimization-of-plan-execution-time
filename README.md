...
# Задача о назначениях - Минимизация времени выполнения плана

## Описание
Веб-приложение для решения задачи о назначениях с возможностью минимизации времени выполнения плана.

## Функциональность
- Ввод матриц C (стоимости) и T (времена) через textarea
- Элементы матрицы вводятся через пробел
- Строки матрицы отделяются переносами строк
- Автоматическое решение задачи о назначениях
- Отображение всех возможных решений
- Выбор оптимального решения

## Как использовать

### Ввод матриц
1. **Матрица C (стоимости)**: Введите элементы матрицы через пробел, строки отделяйте переносами
   ```
   1 2 3
   4 5 6
   7 8 9
...
2. **Матрица T (времена)**: Аналогично введите элементы матрицы времени
   ```
   2 1 3
   1 2 1
   3 2 1
...
### Параметры
- **Фонд зарплаты S**: Максимальная сумма затрат
- **Тип оптимизации T**: min (минимизация) или max (максимизация)
- **Ограничение на T**: Дополнительное ограничение на время (опционально)
...
## Технические детали
- Размер матриц: 3x3 (по умолчанию)
- Алгоритм: Перебор всех возможных перестановок
- Язык: JavaScript (ES6+)
- Стили: SCSS с автоматической компиляцией

## Запуск
1. Откройте `prod/index.html` в браузере
2. Введите матрицы в соответствующие поля
3. Настройте параметры
4. Нажмите "Решить"

## Структура проекта
- `dev/` - исходные файлы для разработки
- `prod/` - готовые файлы для продакшена
