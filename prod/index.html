...
5 9 2 5 8
2 1 4 6 7
7 8 5 3 1</textarea></div></div><div class="params"><label for="salary-fund">Фонд зарплаты S:</label><input id="salary-fund" type="number" name="S" min="0" value="33"><label for="t-type">Тип оптимизации T:</label><select id="t-type" name="T"><option value="min" selected>min</option><option value="max">max</option></select><label for="t-limit">Ограничение на T (Если в условии не указано, оставить пустым):</label><input id="t-limit" type="number" name="T_limit" min="0" step="any"><label><input id="skip-increasing-time" type="checkbox" checked>Не считать время, если оно больше одного из предыдущих времен</label><br><label><input id="hide-invalid-solutions" type="checkbox" checked>Не отображать решения, которые не удовлетворяют ограничениям</label><br><br><h3>Работают правильно:</h3><label><input id="solve-hungarian" type="checkbox" checked>Решить методом Венгера (венгерский алгоритм)</label><br><label><input id="solve-branch" type="checkbox" checked>Решить методом ветвей и границ</label><br><label><input id="solve-lp" type="checkbox" checked>Решить методом линейного программирования</label><br><label><input id="solve-greedy" type="checkbox" checked>Решить жадным методом</label></div><button id="solve-btn" type="button">Решить</button></form><div class="preset-buttons"><button id="fill-default" type="button">Заполнить значениями по умолчанию</button><button id="fill-test" type="button">Заполнить тестовыми значениями</button><button id="clear-all" type="button">Очистить все поля</button></div><section id="solution"><h2>Ответ</h2><p id="answer"></p><h3>Решение полным перебором</h3><div id="solution-table"></div><div id="extra-methods"></div></section></main><script type="module" src="./assets/js/first_script.js"> </script></body></html>